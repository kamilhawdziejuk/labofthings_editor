...
{
    [System.AddIn.AddIn("HomeOS.Hub.Apps.Light")]
    public class AppLight : ModuleBase
    {
        AppLightSvc service; 
        ...
        /// <summary> 
        /// The most recently fetched value from the sensor 
        /// </summary> 
        public int Light = 0;
        ...
        { 
            ...
            service = new AppLightSvc(this, logger);
            serviceHost = AppLightSvc.CreateServiceHost(logger, this, service, moduleInfo.BaseURL() + "/webapp");
            serviceHost.Open();
        ...
        public override void PortRegistered(VPort port)
        {
            lock (this)
            {
                if (Role.ContainsRole(port, RoleSensor.RoleName))
                {
                    ...
                    if (registeredSensors.ContainsKey(port)) registeredSensors[port] = capability; else registeredSensors.Add(port, capability);
                    if (capability != null) { port.Subscribe(RoleSensor.RoleName, RoleSensor.OpGetName, this.ControlPort, capability, this.ControlPortCapability); }
                } if (Role.ContainsRole(port, RoleActuator.RoleName))
                {
                    VCapability capability = GetCapability(port, Constants.UserSystem);
                    if (registeredActuators.ContainsKey(port)) registeredActuators[port] = capability; else registeredActuators.Add(port, capability);
                    if (capability != null) { port.Subscribe(RoleActuator.RoleName, RoleActuator.OpPutName, this.ControlPort, capability, this.ControlPortCapability); }
                }
            }
        }
